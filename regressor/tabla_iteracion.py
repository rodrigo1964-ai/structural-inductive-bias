"""
Tabla reorganizada: Comparación clara CON vs SIN iteración
"""
    ...
    for k in range(start_k, n):
        ...
        if n_points == 3:
            if n_iterations == 0:
                ...
                gpp = (d2f_dy2(y[k], yp_k) + 2 * d2f_dydyp(y[k], yp_k) * 3/(2*T) + d2f_dyp2(y[k], yp_k) * (3/(2*T))**2)
            ...
            if n_iterations == 0:
                ...
                gpp = (d2f_dy2(y[k], yp_k) + 2 * d2f_dydyp(y[k], yp_k) * 11/(6*T) + d2f_dyp2(y[k], yp_k) * (11/(6*T))**2)
                gppp = d3f_dy3(y[k], yp_k) if d3f_dy3 is not None else 0.0
                delta = - g / gp
                if n_terms >= 2:
                    delta += - (1/2) * g**2 * gpp / gp**3
...
if __name__ == "__main__":
    ...
    print("\n" + "="*85)
    print("COMPARACIÓN: CON vs SIN ITERACIÓN (2 términos)")
    print("="*85)
    print(f"{'Ejemplo':<20} {'SIN iter 3pt':<15} {'CON iter 3pt':<15} {'SIN iter 4pt':<15} {'CON iter 4pt':<15}")
    print("-"*85)
    ...
    for nombre, func in ejemplos:
        ...
        sin_4pt = func(2, 0, 4)
        con_4pt = func(2, 1, 4)
        print(f"{nombre:<20} {sin_3pt:<15.2e} {con_3pt:<15.2e} {sin_4pt:<15.2e} {con_4pt:<15.2e}")

    print("="*85)
    print("\n")

    print("="*85)
    print("COMPARACIÓN: CON vs SIN ITERACIÓN (3 términos)")
    print("="*85)
    print(f"{'Ejemplo':<20} {'SIN iter 3pt':<15} {'CON iter 3pt':<15} {'SIN iter 4pt':<15} {'CON iter 4pt':<15}")
    print("-"*85)
